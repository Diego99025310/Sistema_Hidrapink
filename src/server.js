...
const insertImportedInfluencers = db.transaction((rows) => {
  ...
  rows.forEach((row) => {
    ...
    if (row.loginEmail && row.passwordHash) {
      const userResult = insertUserStmt.run(row.loginEmail, row.passwordHash, 'influencer', mustChange);
  ...
  try {
    ...
    if (error && (error.code === 'SQLITE_CONSTRAINT_UNIQUE' || error.code === 'ER_DUP_ENTRY')) {
      return res.status(409).json({
        error: 'Dados duplicados encontrados durante a importação. Verifique os logins e documentos informados.',
        analysis
      });
    }
