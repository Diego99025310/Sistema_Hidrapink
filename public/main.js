(() => {
  ...
  const influencerImportColumns = [
    {
      header: 'Nome',
      label: 'Nome completo',
      note: 'Utilize o nome civil ou artístico completo.',
      required: false
    },
    {
      header: 'Instagram',
      label: 'Perfil no Instagram (com ou sem @)',
      note: 'Informe apenas o usuário, sem espaços.',
      required: false
    },
    {
      header: 'EmailContato',
      label: 'Email principal de contato',
      note: 'Se o campo "Email de acesso" ficar vazio, este email será usado para o login.',
      required: false
    },
    {
      header: 'EmailAcesso',
      label: 'Email de acesso ao sistema',
      note: 'Opcional. Preencha se quiser usar um email diferente do contato.',
      required: false
    },
    {
      header: 'CPF',
      label: 'CPF (apenas números)',
      note: 'Opcional. Informe para gerar a senha provisória automaticamente.',
      required: false
    },
    {
      header: 'Telefone',
      label: 'Telefone/WhatsApp',
      note: 'Opcional. Utilize DDD + número.',
      required: false
    },
    {
      header: 'Cupom',
      label: 'Cupom da influenciadora',
      note: 'Opcional. Informe o cupom ou deixe em branco.',
      required: false
    },
    {
      header: 'ComissaoPercentual',
      label: 'Comissão (%)',
      note: 'Opcional. Valores entre 0 e 100.',
      required: false
    },
    {
      header: 'CEP',
      label: 'CEP',
      note: 'Opcional. Aceita formatos com ou sem máscara.',
      required: false
    },
    {
      header: 'Numero',
    ...
    },
    {
      header: 'Estado',
      label: 'Estado (UF)',
      note: 'Opcional. Utilize a sigla com duas letras.',
      required: false
    }
  ];
  ...
  const initMasterImportInfluencersPage = () => {
    if (!ensureAuth('master')) return;
    attachLogoutButtons();

    const columnList = document.getElementById('influencerColumnList');
    const downloadTemplateBtn = document.getElementById('downloadInfluencerTemplate');
    const fileInput = document.getElementById('influencerImportFile');
    const analyzeBtn = document.getElementById('analyzeInfluencerImport');
    const confirmBtn = document.getElementById('confirmInfluencerImport');
    const clearBtn = document.getElementById('clearInfluencerImport');
    const messageEl = document.getElementById('influencerImportMessage');
    const summaryContainer = document.getElementById('influencerImportSummary');
    const tableBody = document.querySelector('#influencerImportTable tbody');
    const resultContainer = document.getElementById('influencerImportResult');
    ...
    const renderColumns = () => {
      ...
      influencerImportColumns.forEach((column, index) => {
        const item = document.createElement('li');
        item.dataset.required = column.required ? 'true' : 'false';
        item.setAttribute('aria-setsize', String(influencerImportColumns.length));
        item.setAttribute('aria-posinset', String(index + 1));

        const label = document.createElement('span');
        label.className = 'import-column-label';
        label.textContent = `${column.header} — ${column.label}`;
        item.appendChild(label);

        const meta = document.createElement('span');
        meta.className = 'import-column-meta';
        const statusText = column.required ? 'Obrigatório' : 'Opcional';
        meta.textContent = column.note ? `${statusText} • ${column.note}` : statusText;
    ...
    const renderTable = (rows) => {
      ...
      rows.forEach((row) => {
        ...
        tr.dataset.status = hasErrors ? 'error' : 'ok';

        const instagramDisplay = (() => {
          if (!row.instagram) return '-';
          const trimmed = String(row.instagram).trim();
          if (!trimmed) return '-';
        ...
        const cells = [
          ...
          row.cpf || '-',
          row.cupom || '-',
          hasErrors ? row.errors.join(' • ') : 'Pronto para importar'
        ];
        ...
        cells.forEach((value, index) => {
          ...
          if (index === cells.length - 1) {
            td.dataset.type = hasErrors ? 'error' : 'success';
          }
          tr.appendChild(td);
        });
    ...
    const renderSummary = (analysis) => {
      ...
      const errorCount =
        analysis.errorCount != null
          ? analysis.errorCount
          : Math.max((analysis.totalCount || 0) - (analysis.validCount || 0), 0);

      const metrics = [
        {
          label: 'Linhas analisadas',
          value: String(analysis.totalCount || 0),
          icon: 'orders'
        },
        {
          label: 'Prontas para importação',
          value: String(analysis.validCount || 0),
          icon: 'commission',
          helper: analysis.validCount ? 'Todos os registros listados serão importados.' : undefined
        },
        {
          label: 'Com pendências',
          value: String(errorCount),
          icon: 'info',
          helper: errorCount ? 'Revise as linhas destacadas antes de concluir.' : 'Nenhuma pendência encontrada.'
        }
      ];

      renderSummaryMetrics(summaryContainer, metrics);
    };
    ...
    const updateConfirmState = () => {
      ...
      if (lastAnalysis && lastAnalysis.totalCount && !lastAnalysis.hasErrors) {
    ...
    const resetImportView = ({ clearFile = false, clearMessage = true } = {}) => {
      lastAnalysis = null;
      lastImportText = '';
    ...
    const renderResultCredentials = (rows) => {
      if (!resultContainer) return;
      resultContainer.innerHTML = '';
      if (!Array.isArray(rows) || !rows.length) return;

      const title = document.createElement('h3');
      title.textContent = 'Credenciais geradas para compartilhar';
      ...
      rows.forEach((row) => {
        ...
        const instagramDisplay = row.instagram
          ? row.instagram.startsWith('@')
            ? row.instagram
            : `@${row.instagram}`
          : '-';
        name.textContent = `${row.nome || '-'} (${instagramDisplay})`;
        item.appendChild(name);

        const credentials = document.createElement('span');
        credentials.className = 'import-result-credentials';
        credentials.textContent = `Login: ${row.login_email || '-'} • Senha provisória: ${row.senha_provisoria || '-'} • Código de assinatura: ${row.codigo_assinatura || '-'}`;
        item.appendChild(credentials);
    ...
    downloadTemplateBtn?.addEventListener('click', () => {
      const anchor = document.createElement('a');
      anchor.href = 'modelos/influenciadoras-modelo.csv';
      anchor.download = 'modelo-influenciadoras.csv';
      anchor.style.display = 'none';
      document.body.appendChild(anchor);
      anchor.click();
      document.body.removeChild(anchor);
    ...
    fileInput?.addEventListener('change', () => {
      ...
      renderSummary(null);
      resetResult();
      setMessage(messageEl, 'Clique em "Analisar arquivo" para validar os dados selecionados.', 'info');
      updateConfirmState();
    });
    ...
    analyzeBtn?.addEventListener('click', async () => {
      ...
      if (!file) {
        setMessage(messageEl, 'Selecione o arquivo CSV para analisar.', 'error');
        return;
      }

      analyzeBtn.setAttribute('disabled', 'disabled');
      setMessage(messageEl, 'Lendo arquivo selecionado...', 'info');
      resetResult();

      try {
        const text = await readFileAsText(file);
        if (!text.trim()) {
          resetImportView({ clearFile: false, clearMessage: false });
          setMessage(messageEl, 'O arquivo está vazio. Verifique o conteúdo e tente novamente.', 'error');
          return;
        }

        setMessage(messageEl, 'Analisando dados...', 'info');
        const analysis = await apiFetch('/influenciadoras/import/preview', {
          method: 'POST',
          body: { text }
        });
        ...
        renderTable(analysis.rows || []);
        renderSummary(analysis);

        if (!analysis.totalCount) {
          setMessage(messageEl, 'Nenhuma linha de influenciadora foi encontrada no arquivo.', 'info');
          ...
          setMessage(
            messageEl,
            `Encontramos ${errorCount} linha(s) com pendências. Corrija os dados destacados antes de confirmar a importação.`,
            'error'
          );
        } else {
          setMessage(
            messageEl,
            `Todos os ${analysis.validCount} registros estão prontos para importação.`,
            'success'
          );
        }
      } catch (error) {
        ...
        renderTable([]);
        renderSummary(null);
        setMessage(messageEl, error.message || 'Não foi possível analisar o arquivo selecionado.', 'error');
      } finally {
    ...
    confirmBtn?.addEventListener('click', async () => {
      if (!lastAnalysis) {
        setMessage(messageEl, 'Analise o arquivo antes de confirmar a importação.', 'info');
        updateConfirmState();
        return;
      }
      if (lastAnalysis.hasErrors) {
        setMessage(messageEl, 'Há pendências nas linhas analisadas. Corrija-as antes de importar.', 'error');
        updateConfirmState();
        return;
      }

      const file = fileInput?.files?.[0];
      let text = lastImportText || '';
      if (!text && file) {
        try {
          text = await readFileAsText(file);
        } catch (error) {
          setMessage(messageEl, error.message || 'Não foi possível ler o arquivo para importação.', 'error');
          return;
        }
      }
      ...
      confirmBtn.setAttribute('disabled', 'disabled');
      setMessage(messageEl, 'Importando influenciadoras...', 'info');

      try {
        const result = await apiFetch('/influenciadoras/import/confirm', {
          method: 'POST',
          body: { text }
        });
        ...
        setMessage(
          messageEl,
          `Importação concluída! ${result.inserted || 0} influenciadora(s) cadastradas com sucesso.`,
          'success'
        );
        renderTable([]);
        renderSummary(null);
        renderResultCredentials(result.rows || []);
        lastAnalysis = null;
        lastImportText = '';
        ...
        setMessage(messageEl, error.message || 'Não foi possível concluir a importação.', 'error');
      } finally {
        updateConfirmState();
      }
    });
    ...
    clearBtn?.addEventListener('click', () => {
      resetImportView({ clearFile: true });
      setMessage(messageEl, 'Seleção de arquivo limpa.', 'info');
    });

    renderColumns();
    renderTable([]);
    renderSummary(null);
    setMessage(messageEl, 'Selecione o arquivo CSV e clique em "Analisar arquivo" para começar.', 'info');
    updateConfirmState();
  };
  ...
  const bootstrap = () => {
    ...
    const initializers = {
      ...
      'master-import-influencers': initMasterImportInfluencersPage,
